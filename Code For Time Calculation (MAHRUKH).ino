...
const char* Trailertopic="@msg/Trailer";
...
const int ledPin = 16; // Assuming your LED is connected to pin 2
...
float readEnergyInTrailerFromMATLAB();
float timeInTrailerFromMATLAB();

 // New function to read shadow data
...
void loop() {
  ...
  float shadowData = readShadowDataFromMATLAB(); 
  float energyintrailer = readEnergyInTrailerFromMATLAB();
  float timeintrailer = readtimeInTrailerFromMATLAB(); // Updated function call
  ...
  publishParameter(Trailertopic, energyintrailer,timeintrailer, shadowData);
...
float readEnergyInTrailerFromMATLAB() {
  float energyintrailer = 74.626 + random(0.02, 1.5) ;
  return energyintrailer;
}
float readtimeInTrailerFromMATLAB() {
  float timeintrailer = 8.0 ;
  return timeintrailer;
}
